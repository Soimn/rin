...
Lexer__NextToken(Lexer* lexer)
{
	...
	/// Skip all whitespace and comments
	{
		...
		for (;;)
		{
...
#ifdef ODIN_LEXER_PERF_TEST
			else if (*lexer->cursor == '`')
			{
				++lexer->cursor;
				while (*lexer->cursor != 0 && *lexer->cursor != '`') ++lexer->cursor;
				++lexer->cursor;
			}
			else if (*lexer->cursor == '\\')
			{
			...
			else if (*lexer->cursor >= 0x80 || *lexer->cursor == 0x1F)
			{
				++lexer->cursor;
			}
#endif
...
Lexer__ParseNumber(Lexer* lexer)
{
	...
	else
	{
		...
		if (*lexer->cursor == '.' && Char_IsDigit(lexer->cursor[1]) || (*lexer->cursor&0xDF) == 'E')
		{
			...
			{
				++lexer->cursor;
				while (Char_IsDigit(*lexer->cursor) || *lexer->cursor == '_') ++lexer->cursor;
			}
...
Lexer__ParseString(Lexer* lexer, u8 terminator)
{
	...
	while (i < raw.len)
	{
		if (raw.data[i] != '\\') real.data[real.len++] = raw.data[i++];
		else if (raw.len < i+1)
		{
