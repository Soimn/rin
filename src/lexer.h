...
// TODO
#define LEXER_ERROR() __debugbreak()
        ...
        if (c == '_' || Char_IsAlpha(c))
        {
          ...
          String ident_string = {
            ...
            .len  = (u32)(lexer->cursor - start_of_token),
					...
					Ident ident = Ident_FromString(ident_string);

					// TODO: Consider switching to a hash map
					Token_Kind kind = Token_Ident;
					for (umm i = 0; i < ARRAY_LEN(Token_KeywordIdents); ++i)
					{
						if (Ident_Match(ident, Token_KeywordIdents[i]))
						{
							kind = (Token_Kind)(Token__FirstKeyword + i);
							break;
						}
					}
