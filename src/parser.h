...
Parser__ParsePostfixExpression(Parser* state, AST** expr)
{
  ...
  for (;;)
  {
    ...
    {
      AST_Postfix_Expr* deref = PUSH_EXPR(AST_Postfix_Expr, AST_Deref);
      deref->operand = operand;

      *expr = (AST*)deref;
    }
		else if (EAT_TOKEN(Token_Inc))
    {
      AST_Postfix_Expr* post_inc = PUSH_EXPR(AST_Postfix_Expr, AST_PostInc);
      post_inc->operand = operand;

      *expr = (AST*)post_inc;
    }
		else if (EAT_TOKEN(Token_Dec))
    {
      AST_Postfix_Expr* post_dec = PUSH_EXPR(AST_Postfix_Expr, AST_PostDec);
      post_dec->operand = operand;
    ...
    else
    {
      ...
      switch (GET_TOKEN().kind)
      {
        ...
        case Token_Hat:  kind = AST_PointerTo; break;
				case Token_Inc:  kind = AST_PreInc;    break;
				case Token_Dec:  kind = AST_PreDec;    break;
      }
...
Parser__ParseBinaryExpression(Parser* state, AST** expr)
{
	...
	struct { AST_Binary_Expr** slot; umm precedence; } stack[1 + AST_KIND__PastLastBinaryBlockIDX - AST_KIND__FirstBinaryBlockIDX];
	stack[0].slot       = 0;
	stack[0].precedence = U32_MAX;
	stack[1].slot       = (AST_Binary_Expr**)expr;
	...
	while (TOKEN_KIND__IS_BINARY(GET_TOKEN().kind))
	{
		...
		{
			++stack_cur;
			stack[stack_cur].slot = (AST_Binary_Expr**)&(*stack[stack_cur-1].slot)->right;
		}

		stack[stack_cur].precedence = precedence;

		AST_Binary_Expr** slot = stack[stack_cur].slot;
		AST* left = &(*slot)->header;
		AST* right;
		if (!Parser__ParsePrefixExpression(state, &right)) return false;

		*slot = PUSH_EXPR(AST_Binary_Expr, op);
		(*slot)->left  = left;
		(*slot)->right = right;
	}
...
Parser__ParseStatement(Parser* state, bool check_for_semi, AST** stmnt)
{
  if (EAT_TOKEN(Token_Colon))
  {
    ...
    else                                  ((AST_While_Stmnt*)*stmnt)->label = label;
  }
  ...
  else
  {
    ...
    if (EAT_TOKEN(Token_Colon))
    {
      ...
      else
      {
        ...
        if (EAT_TOKEN(Token_EQ))
        {
					if      (EAT_TOKEN(Token_StrikeOut)) is_uninitialized = true;
					else if (!Parser__ParseCommaSeparatedExpressions(state, &values)) return false;
        }
  ...
  if (check_for_semi && (stmnt_kind != AST_Block && stmnt_kind != AST_If && stmnt_kind != AST_While))
  {
...
static bool
Parser_ParseFile(Arena ast_arena, u8* file_contents, AST** ast)
{
  Parser _parser = {
    .ast_arena = ast_arena,
